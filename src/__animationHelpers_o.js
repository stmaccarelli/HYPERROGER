	...
	function moveModel(model){
		if(model.position.z >= -HLE.WORLD_WIDTH){
			...
			if(model.position.y==HL.sea.position.y){
				model.rotation.x = Math.cos(HLE.acceleration*0.003)*0.3 * Math.sin(HLE.acceleration*0.003);
				model.rotation.y = Math.sin(HLE.acceleration*0.003)*0.3 * Math.cos(HLE.acceleration*0.003);;
				model.rotation.z = Math.cos(HLE.acceleration*0.003)*0.3;
