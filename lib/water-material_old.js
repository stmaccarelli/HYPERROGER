...
THREE.ShaderLib['water'] = {
  ...
  uniforms: THREE.UniformsUtils.merge( [
    THREE.UniformsLib[ "fog" ], {
      ...
      "waterReflectivity":{ type: "f", value: 0.5 },
...
//    'vec3 reflectColor = reflectionSample * waterReflectivity + reflectionSample * specularLight;',
   ' vec3 outgoingLight = mix(color, reflectionSample*(1.0-specularLight)*(reflectance-0.5)*2.0 * waterReflectivity + specularLight , .5 + reflectance*.5 ) + vec3(reflectance*.5);',

   ' gl_FragColor = vec4( outgoingLight, alpha + (reflectance*(1.0-alpha)) );',//alpha );',
...
THREE.Water = function (renderer, camera, scene, options) {
  ...
  this.alpha = optionalParameter(options.opacity, 0.7);
  this.time = optionalParameter(options.time, 0.0);
  this.normalSampler = optionalParameter(options.waterNormals, null);
  this.sunDirection = optionalParameter(options.sunDirection, new THREE.Vector3(0.70707, 0.70707, 0.0));
  this.sunColor = new THREE.Color(optionalParameter(options.sunColor, 0xffffff));
  this.color = new THREE.Color(optionalParameter(options.color, 0x000000));
  this.waterReflectivity = optionalParameter(options.waterReflectivity, 0.5);
