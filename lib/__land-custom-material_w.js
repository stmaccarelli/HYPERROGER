var LandMat = {
  ...
  THREE.ShaderChunk['common'],
    ...
    "aaa =         snoise(vec3( (nPos.x - landMotion.x ) / worldWidth * 1.8   , ((nPos.z  - landMotion.z )/worldWidth) * 1.8   , buildFreq*.120 ) ) ;",
    "float nBbb =  snoise(vec3( (nPos.x - landMotion.x ) / worldWidth * 8.0   , ((nPos.z  - landMotion.z )/worldWidth) * 8.0   , buildFreq*.125  ) ) ;",//1.0-distance( vec2(0,0), vec2(position.x,position.z+advanceMod) ) / worldWidth;",
    "float nCcc =  snoise(vec3( (nPos.x - landMotion.x ) / worldWidth * 50.0 , ((nPos.z  - landMotion.z )/worldWidth) * 50.0 , buildFreq*0.5+100.0) ) ;",//1.0-distance( vec2(0,0), vec2(position.x,position.z+advanceMod) ) / worldWidth;",

    "vec2 noises = vec2(",
    "snoise( vec3( (nPos.x - landMotion.x ) / worldWidth * 0.8   , (nPos.z  - landMotion.z ) / worldWidth * 0.8   , 50.0 ) ) ,",
    "snoise( vec3( (nPos.x - landMotion.x ) / worldWidth * 0.28   , (nPos.z  - landMotion.z ) / worldWidth * 0.28   , 100.0 ) ) );",

    "vec2 factors = clamp(vec2(",
    "bFactor * .7 + (noises.r-0.5) *.3,",
    "cFactor * .7 + (noises.g-0.5) *.3), 0.0,1.0);",
  ...
  //  " centerPath = centerPath * (2.5-centerPath) * 2.5;",
    ...
    "nPos.y = (aaa + abs(bbb)*0.5 + abs(ccc)*0.5) * (1.0/(1.0+factors.x+factors.y));",
    // base altitude
...
//    ".4 + snoise( vec3( (vUv.x) *worldTiles/50.0, (vUv.y) * worldTiles/50.0, noiseFreq2*1.05 ))* .7 ,",

      ".3 + snoise( vec3( (vUv.x) *0.5 , (vUv.y) *0.5, noiseFreq2 * 1.1  ))* .7 ,",
      ".3 + snoise( vec3( (vUv.x) *0.5 , (vUv.y) *0.5, noiseFreq2 * 1.05 ))* .7 ,",
      ".3 + snoise( vec3( (vUv.x) *0.5 , (vUv.y) *0.5, noiseFreq2        ))* .7 ) * 1.0  ;",
  ...
  //  " texelColor = mapTexelToLinear( texelColor );",
    ...
    " diffuseColor -= texture2D( map2,vUv );",
    "#endif",
  ...
  //  "float seashore = max( (1.0-abs(altitude)-0.93)*12.0, 0.0);",
    ...
    "gl_FragColor.rgb *= natural+peaks*natural;",
    "gl_FragColor.rgb *= 1.0-rainbow+noiseColor*rainbow;",
    // "gl_FragColor.rgb -= sin(landMotion/worldWidth) * .05;",
