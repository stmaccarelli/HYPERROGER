...
THREE.ShaderLib['water_hy'] = {
  ...
  fragmentShader: [
    ...
    '{',
    '  vec2 uv0 = uv / (103.0 * noiseScale) - vec2(advance / 17.0, advance / 29.0) + vec2(0.0,advance*0.1);',
    '  vec2 uv1 = uv / (107.0 * noiseScale) - vec2(advance / -19.0, advance / 31.0) + vec2(0.0,advance*0.1);',
    '  vec2 uv2 = uv / (vec2(8907.0, 9803.0) * noiseScale) - vec2(advance / 101.0, advance /   97.0) + vec2(0.0,advance*0.02);',
    '  vec2 uv3 = uv / (vec2(1091.0, 1027.0) * noiseScale) - vec2(advance / 109.0, advance / -113.0) + vec2(0.0,advance*0.02);',
    ...
    '  sunLight(distordNormal, eyeDirection, 80.0, 2.0, 0.5, diffuseLight, specularLight);',
    ...
    '   blurSample += texture2DProj(mirrorSampler, mirrorDistord+vec3( 1.0*i, 1.1*i,0.0)).xyz;',
    ' };',
    ' blurSample = (blurSample/8.0/(8.0/4.0)) * waterReflectivity;',
    ...
    ' specularFinal = specularLight*blurSample*vec3(blurSample.r*blurSample.g)*0.6;',
    '}',
    'else{',
   ...
   '}',
    ...
    // 'gl_FragColor = vec4( color * scatter + mirrorFinal + mirrorFinal*color*0.5 + specularFinal, alpha-length(scatter)*0.9 );',//' , alpha + (reflectance*(1.0-alpha)) );',//alpha );',
    'gl_FragColor = vec4( reflectionSample*reflectance+specularFinal,1.0);',//' , alpha + (reflectance*(1.0-alpha)) );',//alpha );',
...
THREE.Water = function (renderer, camera, scene, options) {
  ...
  this.noiseScale = optionalParameter(options.noiseScale, 20.0);
