...
//
  ...
  `
    ...
    vec4 texelColor = mix(texture2D( map, vUv ), texture2D( map2, vUv ), noises[3]*0.5+0.5 );

    // terrain rainbow coloring
    texelColor.rgb += noiseColor * rainbow ;

    diffuseColor = mix( texture2D(mapSand, vUv), texelColor * diffuseColor, clamp( seaShore , 0.0, 1.0) );
