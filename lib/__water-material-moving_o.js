...
THREE.ShaderLib['water_hy'] = {
  ...
  vertexShader:[
    ...
    'void main()',
    '{',
    ' vec3 nPos = position + vec3(0, 0, noise(position.xz*advance)*10.0 );',
  ...
  fragmentShader: [
    ...
    '  vec2 uv0 = uv / (2103.0 * noiseScale) - vec2( 17.0,  29.0) * advance; //+ vec2(0.0,advance*0.01);',
    '  vec2 uv1 = uv / (2107.0 * noiseScale) - vec2(-19.0,  31.0) * advance; //+ vec2(0.0,advance*0.01);',
    '  vec2 uv2 = uv / (2111.0 * noiseScale) - vec2( 11.0,  57.0) * advance; //+ vec2(0.0,advance*0.01);',
    '  vec2 uv3 = uv / (2115.0 * noiseScale) - vec2( 19.0, -23.0) * advance; //+ vec2(0.0,advance*0.01);',
    '  vec4 noise = ',
   ...
   '}',
    ...
    // Compute final pixel color
     'gl_FragColor = vec4( color * scatter + mirrorFinal + mirrorFinal*color*0.5 + specularFinal, alpha-length(scatter)*0.7 );',//' , alpha + (reflectance*(1.0-alpha)) );',//alpha );',
