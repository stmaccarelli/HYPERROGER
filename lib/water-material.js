...
THREE.ShaderLib['water'] = {
  ...
  fragmentShader: [
    ...
    // '  vec2 uv1 = uv / (107.0 * noiseScale) - vec2(time / -40.0, time / 31.0);',
    // '  vec2 uv2 = uv / (vec2(890.070, 880.030) * noiseScale) + vec2(time / 500.0, time /   10.0);',
    // '  vec2 uv3 = uv / (vec2(50.10, 50.70) * noiseScale) + vec2(time / 500.90, time /  10.30);',
    ...
    '  vec3 distordNormal = (distordCoord + surfaceZ)*1.5;',
  ...
  //  '  vec3 albedo = mix(sunColor * diffuseLight * 0.3 + scatter, (vec3(0.1) + reflectionSample * waterReflectivity + reflectionSample * specularLight), reflectance);',
//    'vec3 reflectColor = reflectionSample * waterReflectivity + reflectionSample * specularLight;',

//' vec3 outgoingLight = mix(color + specularLight, (reflectionSample * waterReflectivity * reflectance + reflectionSample * specularLight*reflectance)*0.0, .5 + (reflectance-0.5)*2.0 ) + vec3((reflectance-0.5)*2.0*reflectionSample*1.0) + specularLight*sunColor*waterReflectivity*1.0;',
' vec3 outgoingLight = mix(color + specularLight, (reflectionSample * waterReflectivity * (reflectance-0.5)*2.0 + reflectionSample * specularLight*(reflectance-0.25)*1.5), .5 + (reflectance-0.25)*1.5 ) + specularLight*reflectionSample*waterReflectivity*1.0;',
//vec3(reflectance)*reflectionSample+color
'vec3 mirrorFinal=(reflectance-scatter*2.0)*reflectionSample - specularLight*scatter;',
'float refSampleLuma = (reflectionSample.r+reflectionSample.g+reflectionSample.b) / 3.0;',
   ' gl_FragColor = vec4( color * scatter * 5.0 + mirrorFinal*0.5 + mirrorFinal*color*0.5  + specularLight*mirrorFinal, 0.9-length(scatter)*0.7 );',//' , alpha + (reflectance*(1.0-alpha)) );',//alpha );',
...
THREE.Water = function (renderer, camera, scene, options) {
  ...
  this.alpha = optionalParameter(options.opacity, 0.9);
