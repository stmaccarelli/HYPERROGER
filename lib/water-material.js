...
THREE.ShaderLib['water_hy'] = {
  ...
  fragmentShader: [
    ...
    '  vec3 noise = getNoise(modelPosition.xy);',
    '  vec3 distordCoord = noise.x * surfaceX + noise.y * surfaceY;',
    '  vec3 distordNormal = (distordCoord + surfaceZ)*4.0;',
    ...
    ' vec3 mirrorDistord = mirrorCoord.xyz + vec3(distortion.x, distortion.y-2.0, 1.0);',
   ...
   '}',
    ...
    'gl_FragColor = vec4( color * scatter * 5.0 + mirrorFinal + mirrorFinal*color*0.5 + specularFinal, alpha-length(scatter)*0.7 );',//' , alpha + (reflectance*(1.0-alpha)) );',//alpha );',
    ...
    // "	#else",
    // "		float fogFactor = smoothstep( fogNear, fogFar/5.0, depth );",
    // " #endif",
...
THREE.Water = function (renderer, camera, scene, options) {
  ...
  this.scene = scene;
  this.scene.fog = new THREE.Fog(new THREE.Color(0xff0000), 0.0 , - height * 0.45);
