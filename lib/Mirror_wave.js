...
THREE.ShaderLib[ 'mirrorWave' ] = {
	...
	vertexShader: [
		...
		"float posY = 2.0 + sin( time * .2 + nPos.x*12.0 + nPos.z*13.5) * seaHeight;",
    ...
    THREE.ShaderChunk[ "fog_pars_fragment" ],
		"void main() {",
			...
			'vec3 reflectionSample = texture2DProj(mirrorSampler, mirrorCoord).rgb;',
		 	...
		 	"vec4 color = vec4( mix(mirrorColor,reflectionSample * 0.65,0.5), opacity);",
			// "vec4 color = vec4(mirrorColor + reflectionSample * 0.6, opacity);",
