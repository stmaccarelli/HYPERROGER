var LandMat = {
  ...
  THREE.ShaderChunk['common'],
    ...
    "float nBbb =  snoise(vec3( (position.x / worldWidth) * 8.0  , ((posZ - advance)/worldWidth) * 8.0   , buildFreq*.5 ) ) ;",//1.0-distance( vec2(0,0), vec2(position.x,position.z+advanceMod) ) / worldWidth;",
    "float nCcc =  snoise(vec3( (position.x / worldWidth) * 100.0, ((posZ - advance)/worldWidth) * 100.0 , 100.0 ) ) ;",//1.0-distance( vec2(0,0), vec2(position.x,position.z+advanceMod) ) / worldWidth;",
  ...
  //  "aaa = (aaa+1.0)/2.0;",
    ...
    "noiseColor = vec3(nBbb*aaa-nCcc, nCcc, nCcc*nBbb) ;",
      //
