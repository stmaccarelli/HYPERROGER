var LandMat = {
  ...
  THREE.ShaderChunk['common'],
    ...
    "aaa = floor( snoise25d(vec3( (position.x / worldWidth) * 2.0  , ((posZ - advance)/worldWidth) * 2.0   , buildFreq*.5 ) ) /squareness)*squareness;",
    "bbb = floor( snoise25d(vec3( (position.x / worldWidth) * 8.0  , ((posZ - advance)/worldWidth) * 8.0   , buildFreq*.5 ) ) /squareness)*squareness;",//1.0-distance( vec2(0,0), vec2(position.x,position.z+advanceMod) ) / worldWidth;",
    "ccc = floor( snoise25d(vec3( (position.x / worldWidth) * 100.0, ((posZ - advance)/worldWidth) * 100.0 , 100.0        ) ) /squareness)*squareness;",//1.0-distance( vec2(0,0), vec2(position.x,position.z+advanceMod) ) / worldWidth;",
  ...
  //  "aaa = (aaa+1.0)/2.0;",
    ...
    "noiseColor = vec3(",
      ".3 + snoise25d( vec3( (uv.x) *worldTiles/50.0, (uv.y+advanceUV) * worldTiles/50.0, noiseFreq*1.2 ))* .7 ,",
      ".3 + snoise25d( vec3( (uv.x) *worldTiles/50.0, (uv.y+advanceUV) * worldTiles/50.0, noiseFreq*1.1 ))* .7 ,",
      ".3 + snoise25d( vec3( (uv.x) *worldTiles/50.0, (uv.y+advanceUV) * worldTiles/50.0, noiseFreq     ))* .7 ) ;",
      // "vec3(aaa,bbb,ccc);",
