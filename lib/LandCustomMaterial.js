var LandMat = {
  ...
  //  "aaa = (aaa+1.0)/2.0;",
    ...
    "noiseColor = vec3(",
      "snoise( vec3( (uv.x) *worldTiles/50.0, (uv.y+advanceUV) * worldTiles/50.0, 10.0 )) ,",
      "snoise( vec3( (uv.x) *worldTiles/50.0, (uv.y+advanceUV) * worldTiles/50.0, 11.0 )) ,",
      "snoise( vec3( (uv.x) *worldTiles/50.0, (uv.y+advanceUV) * worldTiles/50.0, noiseFreq)) )+",
      "vec3(aaa,ccc,ddd);",
  ...
  //  "float seashore = max( (1.0-abs(altitude)-0.93)*12.0, 0.0);",
    ...
    // "gl_FragColor.rgb *=  (altitude>0.0?(ccc*1.0+0.6):1.0);",
    // "gl_FragColor.rgb *=  (altitude>0.0?(ddd*1.6+0.5):1.0);",

    "float peaks = (altitude+1.0) * ",
    "  (altitude>0.0?(ccc*1.0+0.6):1.0) *",
    "  (altitude>0.0?(ddd*1.6+0.5):1.0) ;",

    "gl_FragColor.rgb *= 1.0-natural+peaks*natural;",
    "gl_FragColor.rgb *= noiseColor*rainbow+1.0-rainbow;",
    // "gl_FragColor.rgb = Uncharted2ToneMapping(gl_FragColor.rgb);",
    // "gl_FragColor.rgb = mix(gl_FragColor.rgb,noiseColor,rainbow);",

    //
    // "float peaks = (altitude+2.0)*0.5 * altitude>0.0?(ccc*1.0+0.6):1.0 * altitude>0.0?(ddd*1.6+0.5):1.0;",
    //
    ...
    // "gl_FragColor.rgb =  vec3(ccc*1.0+0.6);",
