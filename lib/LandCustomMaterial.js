var LandMat = {
  ...
  THREE.ShaderChunk['common'],
    ...
    "bbb =  snoise(vec3( (position.x / worldWidth) * 8.0  , ((posZ - advance)/worldWidth) * 8.0   , buildFreq*.5 ) ) ;",//1.0-distance( vec2(0,0), vec2(position.x,position.z+advanceMod) ) / worldWidth;",
    "ccc =  snoise(vec3( (position.x / worldWidth) * 100.0, ((posZ - advance)/worldWidth) * 100.0 , 100.0        ) ) ;",//1.0-distance( vec2(0,0), vec2(position.x,position.z+advanceMod) ) / worldWidth;",
    ...
    "float centerPath = abs(position.x/worldWidth*0.5) * 4.0;",

    "float posY = (aaa + bbb + ccc*0.5) * (1.0/(1.0+bFactor+cFactor));",
  ...
  //  "aaa = (aaa+1.0)/2.0;",
    ...
    "noiseColor = vec3(",
      ".4 + snoise( vec3( (uv.x) *worldTiles/50.0, (uv.y+advanceUV) * worldTiles/50.0, noiseFreq*1.1 ))* .7 ,",
      ".4 + snoise( vec3( (uv.x) *worldTiles/50.0, (uv.y+advanceUV) * worldTiles/50.0, noiseFreq*1.05 ))* .7 ,",
      ".4 + snoise( vec3( (uv.x) *worldTiles/50.0, (uv.y+advanceUV) * worldTiles/50.0, noiseFreq     ))* .7 ) ;",
      // "vec3(aaa,bbb,ccc);",
    ...
    "altitudeWithCenterPath = (posY * (withCenterPath?centerPath:1.0) / landHeight) ;",// / (poss.y<0.0?0.4:2.0) );",
  ...
  //  "float seashore = max( (1.0-abs(altitude)-0.93)*12.0, 0.0);",
    ...
    "gl_FragColor.rgb -= seashore*worldColor*peaks;",
    "gl_FragColor *= seaDepth;",
