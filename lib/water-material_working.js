...
THREE.ShaderLib['water_hy'] = {
  ...
  fragmentShader: [
    ...
    ' vec3 mirrorDistord = mirrorCoord.xyz + vec3(distortion.x, distortion.y-0.0, 1.0);',
    ...
    // '   blurSample += texture2DProj(mirrorSampler, mirrorDistord+vec3( 1.0*i, 1.1*i,0.0)).xyz;',
    // ' };',
    // ' blurSample = (blurSample/8.0/(8.0/2.0));',
    //
  ...
  //  '}',
    ...
    'gl_FragColor = vec4( color * scatter * 5.0 + mirrorFinal*0.5 + mirrorFinal*color*0.5 + specularFinal, 0.9-length(scatter)*0.7 );',//' , alpha + (reflectance*(1.0-alpha)) );',//alpha );',
