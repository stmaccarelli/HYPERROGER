...
var NUMFREQS = 512;
var freqArray = [];
var tempFreq = new Float32Array(NUMFREQS);
...
function fftArray() {
  // if(source.playbackState==1) source.start();
    ...
    analyserNode.getFloatFrequencyData(tempFreq);
  //  var multiplier = analyserNode.frequencyBinCount / numFreqs;

    // Draw rectangle for each frequency bin.
     for (var i = 0; i < analyserNode.frequencyBinCount; ++i)
    //  freqArray[i] = tempFreq[i] - analyserNode.minDecibels;
      freqArray[i] = (tempFreq[i] - analyserNode.minDecibels) / (analyserNode.maxDecibels - analyserNode.minDecibels);
